using UnityEditor;
using UnityEngine;

namespace LunraGames.SubLight
{
	[CustomPropertyDrawer(typeof(CurveRange))]
	public class CurveRangeDrawer : PropertyDrawer
	{
		const float RowHeight = 16f;
		const float RowSpacing = 2f;

		const float IndentWidth = 16f;
		const float CurveWidth = 80f;
		const float ColumnSpacing = 2f;
		const float CurveTotalWidth = CurveWidth + ColumnSpacing;

		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			return (RowHeight * 2f) + RowSpacing;
		}

		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			// Using BeginProperty / EndProperty on the parent property means that
			// prefab override logic works on the entire property.
			label = EditorGUI.BeginProperty(position, label, property);

			// Don't make child fields be indented
			var indent = EditorGUI.indentLevel;
			EditorGUI.indentLevel = 0;

			// Calculate rects
			var labelRect = new Rect(position.x, position.y, position.width, RowHeight);
			var rangeRect = new Rect(position.x + IndentWidth, labelRect.yMax + RowSpacing, position.width - CurveTotalWidth - IndentWidth, RowHeight);
			var curveRect = new Rect(rangeRect.xMax + ColumnSpacing, rangeRect.y, CurveWidth, RowHeight);

			EditorGUI.LabelField(labelRect, label);
			EditorGUI.indentLevel++;

			EditorGUI.PropertyField(rangeRect, property.FindPropertyRelative("range"), GUIContent.none);
			EditorGUI.PropertyField(curveRect, property.FindPropertyRelative("curve"), GUIContent.none);

			// Set indent back to what it was
			EditorGUI.indentLevel = indent;

			EditorGUI.EndProperty();
		}
	}
}